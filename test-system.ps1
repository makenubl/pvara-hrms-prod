...
if ($mongoContainer -like "*Up*") {
    ...
    Write-Host "  Run: docker-compose up -d mongodb`n" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "  Start backend in new window or run: cd backend; node server.js`n" -ForegroundColor Yellow
}
...
try {
    ...
    if ($frontend -match "200 OK") {
        ...
        Write-Host "✗ Frontend returned unexpected status`n" -ForegroundColor Red
    }
} catch {
    Write-Host "✗ Frontend is NOT responding" -ForegroundColor Red
    Write-Host "  Start frontend: npm run dev`n" -ForegroundColor Yellow
}

# Test 4: Login API
Write-Host "Test 4: Login API Endpoint" -ForegroundColor Yellow
try {
    $loginResponse = curl.exe -s -X POST http://localhost:5000/api/auth/login `
        -H "Content-Type: application/json" `
        -d '{"email":"admin@pvara.com","password":"admin123"}' | ConvertFrom-Json
    ...
    if ($loginResponse.token) {
        ...
        Write-Host "✗ Login failed - no token returned`n" -ForegroundColor Red
    }
} catch {
...
try {
    ...
    if ($usersOutput -match "admin@pvara.com") {
        Write-Host "✓ Admin user exists in database`n" -ForegroundColor Green
    } else {
        Write-Host "✗ Admin user not found" -ForegroundColor Red
        Write-Host "  Run: node backend/scripts/seed.js`n" -ForegroundColor Yellow
    }
} catch {
    Write-Host "✗ Could not check database users`n" -ForegroundColor Red
}
...
Write-Host "  Email:    admin@pvara.com" -ForegroundColor Gray
...
Write-Host "If login is failing in the browser:" -ForegroundColor Yellow
